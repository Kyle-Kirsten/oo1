...
public class Term {
    ...
    public static Term strToTerm(String inStr) {
        if (inStr == null) {
            return null;
        }
        ...
        boolean sign = true;
        while (m.find()) {
            if (m.start() >= 2 && (str).startsWith("**", m.start() - 2)) {
                continue;
            }
            sign = isPos(m.group(1)) == sign;
        ...
        while (m.find()) {
            sign = isPos(m.group(1)) == sign;
            BigInteger tmp = m.group(3) == null || m.group(3).length() == 0 ?
                    BigInteger.valueOf(1) : new BigInteger(m.group(3));
    ...
    public static boolean isPos(String inStr) {
        //检测正负号
        if (inStr == null) {
            return true;
        }
